...
type COTTab = 'overview' | 'markets' | 'extremes' | 'flows' | 'watchlist' | 'spreads' | 'reports';
type ReportType = 'disagg' | 'legacy' | 'tff' | 'cit';

const TABS: ReadonlyArray<{ id: COTTab; label: string }> = [
...
const rowsAsOf = (rows: COTLiveRow[], latestRows: COTLiveRow[], dateKey: string | null) => {
  ...
  return exactRows.length ? exactRows : latestRows;
};
...
const flowAcceleration = (row: COTLiveRow) => {
  const weekly = row.week, fourAvg = row.fourWeek / 4;
...
const timeAtExtreme = (history: COTLiveRow[], ticker: string) => {
  ...
  let count = 0;
  for (const r of series) { if (r.pctRank >= 80 || r.pctRank <= 20) count++; else break; }
  return count;
};
const divergenceFlag = (row: COTLiveRow) => {
  const mmDir = Math.sign(row.managedMoney), flowDir = Math.sign(row.fourWeek);
...
const nextCFTCRelease = () => {
  ...
  return { target, days: Math.floor(diff / 86400000), hours: Math.floor((diff % 86400000) / 3600000), mins: Math.floor((diff % 3600000) / 60000) };
};
const groupRows = (rows: COTLiveRow[]) => Object.values(rows.reduce<Record<string, { asset: string; openInterest: number; managedMoney: number; commercials: number; week: number; count: number }>>((acc, row) => {
  ...
  acc[row.asset].openInterest += row.openInterest; acc[row.asset].managedMoney += row.managedMoney;
  acc[row.asset].commercials  += row.commercials;  acc[row.asset].week += row.week; acc[row.asset].count += 1;
  return acc;
}, {}));
...
const loadWatchlist = (): string[] => { try { const raw = localStorage.getItem(WATCHLIST_KEY); return raw ? JSON.parse(raw) : ['GC', 'CL', 'ZN', 'ES']; } catch { return []; } };
const saveWatchlist = (list: string[]) => { try { localStorage.setItem(WATCHLIST_KEY, JSON.stringify(list)); } catch {} };

const downloadCSV = (filename: string, rows: Record<string, unknown>[]) => {
  if (!rows.length) return;
  const keys = Object.keys(rows[0]);
  const escape = (v: unknown) => { const s = v === null || v === undefined ? '' : String(v); return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; };
...
const buildSnapshotText = (rows: COTLiveRow[], reads: ReturnType<typeof detailReadRows>, dateKey: string | null) => {
  ...
  lines.push(''); lines.push('EXTREMES (>=80 / <=20 %ile):');
...
function Panel({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div className="border border-border overflow-hidden bg-surface-deep">
...
function SortableFilterTable<T>({ rows, columns, rowKey, onRowClick, minWidth = 760, emptyText = 'No matching rows', pinPredicate }: { rows: T[]; columns: TableColumn<T>[]; rowKey: (row: T, index: number) => string; onRowClick?: (row: T) => void; minWidth?: number; emptyText?: string; pinPredicate?: (row: T) => boolean }) {
  ...
  const activeRows = useMemo(() => {
    ...
    return [...sorted.filter(pinPredicate), ...sorted.filter(r => !pinPredicate(r))];
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-[10px] font-mono" style={{ minWidth }}>
        <thead>
          ...
          <tr className="border-b border-border bg-surface-deep/60">
            {columns.map(column => <th key={`${column.key}-filter`} className="px-1 py-1"><input value={filters[column.key] ?? ''} onChange={(event) => setFilters(current => ({ ...current, [column.key]: event.target.value }))} onClick={(event) => event.stopPropagation()} placeholder="FILTER" className={`h-5 w-full border border-border bg-surface-deep px-1 text-[9px] font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent ${column.align === 'right' ? 'text-right' : 'text-left'}`} /></th>)}
          </tr>
        </thead>
        <tbody>
          {activeRows.map((row, index) => <tr key={rowKey(row, index)} onClick={() => onRowClick?.(row)} className={`border-b border-border/40 ${onRowClick ? 'cursor-pointer hover:bg-surface-elevated' : ''} ${pinPredicate?.(row) ? 'bg-accent/5' : index % 2 ? 'bg-surface-elevated/20' : ''}`}>{columns.map(column => <td key={column.key} className={`${column.align === 'right' ? 'text-right' : 'text-left'} px-2 py-1 ${column.className?.(row) ?? 'text-foreground'}`}>{column.render?.(row) ?? column.value(row)}</td>)}</tr>)}
...
export default function COTData() {
  ...
  const [activeReport, setActiveReport] = useState<ReportType>('disagg');
  ...
  const cotRows = historyRows.length
    ? rowsAsOf(historyRows, live.rows, effectiveDate)
    : (live.rows.length ? live.rows : fallbackCotRows);
  const overviewTrend = historyRows.length
    ? buildStoredOverviewTrend(historyRows.filter(row => !effectiveDate || String(row.reportDate).slice(0, 10) <= effectiveDate))
    : trend;
  ...
  const decileCounts = Array.from({ length: 10 }, (_, i) => {
    const lo = i * 10, hi = i === 9 ? 101 : lo + 10;
    return cotRows.filter(r => r.pctRank >= lo && r.pctRank < hi).length;
  });
  const maxDecile = Math.max(...decileCounts, 1);
  ...
  const exportCurrent = () => {
    downloadCSV(`cot-${effectiveDate ?? 'latest'}.csv`, cotRows.map(r => ({
  ...
  return (
    <CmdShell
      code="COT"
      title="Commitments of Traders — CFTC Positioning Monitor"
      headerRight={
        <div className="flex items-center gap-1.5 flex-wrap">
          <button onClick={exportCurrent} className="h-6 border border-border bg-surface-deep px-2 font-mono text-[9px] text-foreground hover:border-accent hover:bg-surface-elevated flex items-center gap-1">
            <Download className="h-3 w-3 text-accent" /> CSV
          </button>
          <button onClick={copySnapshot} className="h-6 border border-border bg-surface-deep px-2 font-mono text-[9px] text-foreground hover:border-accent hover:bg-surface-elevated flex items-center gap-1">
            <CopyIcon className="h-3 w-3 text-accent" /> {snapshotCopied ? 'COPIED' : 'SNAP'}
          </button>
          <Popover>
            <PopoverTrigger asChild>
              <button className="h-6 border border-border bg-surface-deep px-2 font-mono text-[9px] text-foreground hover:border-accent hover:bg-surface-elevated flex items-center gap-1">
          ...
          <button onClick={() => setAsOfDate(null)} className="h-6 border border-border bg-surface-deep px-2 font-mono text-[9px] text-muted-foreground hover:text-accent hover:border-accent">LATEST</button>
          <span className="text-[8px] font-mono text-muted-foreground/70">
            {live.loading ? 'Loading CFTC…' : live.error ? 'Fallback data' : `${live.source} · next ${release.days}d ${release.hours}h`}
          </span>
        </div>
      }
      tabs={
        <CmdTabs
          tabs={TABS}
          active={activeTab}
          onChange={(id) => setActiveTab(id as COTTab)}
          right={<span className="text-[8px] font-mono text-muted-foreground px-2">★ {watchlist.length} · {cotRows.length} contracts</span>}
        />
      }
      kpis={
        <div className="grid grid-cols-6 gap-[1px] bg-border">
          <KpiBox label="TOTAL OI"         value={formatOI(totalOpenInterest)} sub="Cross-market board"          tone="text-foreground" />
          <KpiBox label="SPEC NET"          value={formatK(specNet)}            sub={`${formatK(weeklyNet)} 1W`}  tone={specNet >= 0 ? 'text-positive' : 'text-negative'} />
          <KpiBox label="BULL / BEAR"       value={`${reads.bullish} / ${reads.bearish}`} sub={`of ${cotRows.length} markets`} tone="text-foreground" />
          <KpiBox label="ADDING / CUTTING"  value={`${reads.added} / ${reads.reduced}`}   sub="1W flow direction"  tone={reads.added > reads.reduced ? 'text-positive' : 'text-negative'} />
          <KpiBox label="EXTREME READS"     value={String(extremes)}            sub="≥80 or ≤20 %ile"            tone={extremes > 6 ? 'text-accent' : 'text-foreground'} />
          <KpiBox label="NEXT RELEASE"      value={`${release.days}d ${release.hours}h`} sub="Fri 15:30 ET"      tone="text-foreground" />
        </div>
      }
      footerLeft="Source: CFTC · publicreporting.cftc.gov · Disaggregated Futures Report · 6-hr cache"
      footerRight={`Report: ${effectiveDate ?? 'latest'} · ★ ${watchlist.length} starred · F to star/unstar`}
    >
      <div className="h-full overflow-y-auto">
        <div className="p-3 space-y-3">
          {/* Always-visible selected contract bar */}
          ...
          {activeTab === 'overview' && (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                {([
                  { label: 'TOP SPEC LONG',     value: `${reads.topLong?.ticker  ?? '--'} ${reads.topLong  ? formatK(reads.topLong.managedMoney)  : ''}`, sub: reads.topLong?.market  ?? '', tab: 'markets'  as COTTab, ticker: reads.topLong?.ticker  },
                  { label: 'TOP SPEC SHORT',    value: `${reads.topShort?.ticker ?? '--'} ${reads.topShort ? formatK(reads.topShort.managedMoney) : ''}`, sub: reads.topShort?.market ?? '', tab: 'markets'  as COTTab, ticker: reads.topShort?.ticker },
                  { label: 'BIGGEST 1W FLOW',   value: `${reads.topFlow?.ticker  ?? '--'} ${reads.topFlow  ? formatK(reads.topFlow.week)           : ''}`, sub: reads.topFlow  ? squeezeRead(reads.topFlow)  : '', tab: 'flows' as COTTab, ticker: reads.topFlow?.ticker  },
                  { label: 'LARGEST OI MARKET', value: `${reads.topOi?.ticker    ?? '--'} ${reads.topOi    ? formatOI(reads.topOi.openInterest)    : ''}`, sub: reads.topOi ? `${formatAbsPct(reads.topOi.openInterest / Math.max(1, reads.totalOi) * 100)} of board OI` : '', tab: 'markets' as COTTab, ticker: reads.topOi?.ticker },
                ] as const).map(item => (
                  <button key={item.label} onClick={() => { if (item.ticker) setSelectedTicker(item.ticker); setActiveTab(item.tab); }} className="border border-border bg-surface-deep p-2 text-left hover:bg-surface-elevated hover:border-accent/50 transition-colors">
              ...
              <Panel title="Sector 1W Net Flow — Managed Money">
                <ExpandableResponsiveContainer width="100%" height={150}>
                  <BarChart data={assetRows.map(a => ({ asset: a.asset, week: a.week }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                    <XAxis dataKey="asset" tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} />
                    <YAxis tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v / 1000}K`} />
                    <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', fontFamily: 'monospace', fontSize: 10 }} formatter={(v: number) => [formatK(v), '1W Net']} />
                    <Bar dataKey="week">{assetRows.map((a, i) => <Cell key={i} fill={a.week >= 0 ? 'hsl(var(--positive)/0.7)' : 'hsl(var(--negative)/0.7)'} />)}</Bar>
                  </BarChart>
                </ExpandableResponsiveContainer>
              </Panel>

              <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
                <div className="border border-border bg-surface-deep p-3 xl:col-span-2">
                  <div className="text-[10px] font-mono text-muted-foreground mb-1">Managed Money Net — GC · CL · ZN · ES (Thousands)</div>
                  <ExpandableResponsiveContainer width="100%" height={190}>
                    <LineChart data={overviewTrend}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="week"  tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} />
                      <YAxis tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v}K`} />
                      <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', fontFamily: 'monospace', fontSize: 10 }} formatter={(v: number) => [`${v}K`]} />
                      <Line type="monotone" dataKey="gold"     stroke="hsl(var(--accent))"           strokeWidth={2} dot={false} name="Gold (GC)" />
                      <Line type="monotone" dataKey="crude"    stroke="hsl(var(--positive))"         strokeWidth={2} dot={false} name="Crude (CL)" />
                      <Line type="monotone" dataKey="rates"    stroke="hsl(var(--negative))"         strokeWidth={2} dot={false} name="10Y (ZN)" />
                      <Line type="monotone" dataKey="equities" stroke="hsl(var(--muted-foreground))" strokeWidth={2} dot={false} name="S&P (ES)" />
                    </LineChart>
                  </ExpandableResponsiveContainer>
                </div>
                <div className="border border-border bg-surface-deep p-3">
                  <div className="text-[10px] font-mono text-muted-foreground mb-1">Weekly Net Change — Top 10</div>
                  <ExpandableResponsiveContainer width="100%" height={190}>
                    <BarChart data={cotRows.slice(0, 10)}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="ticker" tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} />
                      <YAxis tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v / 1000}K`} />
                      <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', fontFamily: 'monospace', fontSize: 10 }} formatter={(v: number) => [formatK(v), '1W']} />
                      <Bar dataKey="week">{cotRows.slice(0, 10).map((r, i) => <Cell key={i} fill={r.week >= 0 ? 'hsl(var(--positive)/0.7)' : 'hsl(var(--negative)/0.7)'} />)}</Bar>
                    </BarChart>
                  </ExpandableResponsiveContainer>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                <ShiftTable rows={largestShifts.slice(0, 8)} onSelect={setSelectedTicker} />
              </div>
          ...
          {activeTab === 'extremes' && (
            <div className="space-y-3">
              <Panel title="Positioning Distribution — Contracts by Percentile Decile">
                <div className="flex gap-1.5 px-3 pt-2 pb-3 items-end" style={{ height: 110 }}>
                  {decileCounts.map((count, i) => {
                    const lo = i * 10;
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                        <div className="w-full relative" style={{ height: 72 }}>
                          <div className={`absolute bottom-0 w-full ${lo <= 20 ? 'bg-negative/55' : lo >= 80 ? 'bg-positive/55' : 'bg-accent/30'}`} style={{ height: `${(count / maxDecile) * 100}%` }} />
                        </div>
                        <span className="text-[7px] font-mono text-muted-foreground/60">{lo}s</span>
                        <span className={`text-[9px] font-mono font-bold ${lo <= 20 || lo >= 80 ? 'text-accent' : 'text-muted-foreground'}`}>{count}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                <Panel title="Time at Extreme Monitor">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2">
                    {extremeRows.length > 0 ? extremeRows.map(row => {
                      const tae = timeAtExtreme(historyRows, row.ticker);
                      return (
                        <button key={row.ticker} onClick={() => setSelectedTicker(row.ticker)} className="border border-border bg-surface-elevated/30 p-2 text-left hover:border-accent/60 hover:bg-surface-elevated transition-colors">
                      ...
                      <div className="col-span-2 p-4 text-center text-[9px] font-mono text-muted-foreground">No contracts at extremes currently.</div>
                    )}
                  </div>
                </Panel>
              </div>
          ...
          {activeTab === 'flows' && (
            <div className="space-y-3">
              <FlowReport rows={cotRows} onSelect={setSelectedTicker} />
              <SeasonalReport rows={cotRows} onSelect={setSelectedTicker} />
            </div>
          )}
          ...
          {activeTab === 'spreads' && (
            <div className="space-y-3">
              <SpreadReport rows={liveSpreads} onSelect={setSelectedTicker} />
          ...
          {activeTab === 'reports' && (
            <div className="space-y-3">
              <div className="flex items-center gap-1 flex-wrap border-b border-border pb-2">
                {([
                  { key: 'disagg' as const, label: 'Disaggregated',       desc: 'Producer · Swap Dealer · Managed Money · Other Reportable — most granular' },
                  { key: 'legacy' as const, label: 'Legacy Futures',       desc: 'Commercial · Non-Commercial · Non-Reportable — broad split' },
                  { key: 'tff'    as const, label: 'Fin. Futures (TFF)',   desc: 'Dealer · Asset Manager · Leveraged Funds · Other — financial instruments' },
                  { key: 'cit'    as const, label: 'Index Traders (CIT)',  desc: 'Commodity Index Trader concentration — commodity allocation' },
                ]).map(r => (
                  <button key={r.key} onClick={() => setActiveReport(r.key)} className={`px-3 py-1 border text-[10px] font-mono uppercase transition-colors ${activeReport === r.key ? 'bg-accent text-background border-accent font-bold' : 'border-border text-muted-foreground hover:text-foreground hover:border-accent/50'}`}>
                    {r.label}
                  </button>
                ))}
                <span className="ml-2 text-[8px] font-mono text-muted-foreground/60 hidden lg:block">
                  {activeReport === 'disagg' ? 'Producer · Swap Dealer · Managed Money · Other Reportable — most granular' : activeReport === 'legacy' ? 'Commercial · Non-Commercial · Non-Reportable — broad split' : activeReport === 'tff' ? 'Dealer · Asset Manager · Leveraged Funds · Other — financial instruments only' : 'Commodity Index Trader concentration — index fund vs active trader split'}
                </span>
              </div>
              {activeReport === 'disagg' && <DisaggReport rows={disaggRows} onSelect={setSelectedTicker} />}
              {activeReport === 'legacy' && <LegacyReport rows={legacyRows} onSelect={setSelectedTicker} />}
              {activeReport === 'tff'    && <TFFReport    rows={tffRows}    onSelect={setSelectedTicker} />}
              {activeReport === 'cit'    && <CITReport    rows={citRows}    onSelect={setSelectedTicker} />}
            </div>
          )}
        </div>
      </div>
...
function SelectedContractPanel({ row, onOpenMarkets, starred, onToggleStar }: { row: COTLiveRow; onOpenMarkets: () => void; starred: boolean; onToggleStar: () => void }) {
  return (
    <div className="w-full border border-accent/40 bg-surface-deep p-3 text-left">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {signalCards.map(card => (
        <button key={card.ticker} onClick={() => onSelect(card.ticker)} className="border border-border bg-surface-deep p-2 text-left hover:bg-surface-elevated hover:border-accent/50 transition-colors">
...
function OverviewDetailGrid({ rows, reads, onSelect }: { rows: COTLiveRow[]; reads: ReturnType<typeof detailReadRows>; onSelect: (ticker: string) => void }) {
  ...
  const breadthRows = groupRows(rows).map(row => ({
    asset: row.asset, contracts: row.count,
    oiShare: row.openInterest / totalOi * 100, mmNet: row.managedMoney, oneWeek: row.week,
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
      <Panel title="Board Breadth / Participation">
        <div className="grid grid-cols-2 gap-2 p-2">
          <div className="border border-border bg-surface-elevated/30 p-2"><div className="text-[9px] font-mono text-muted-foreground">BULL / BEAR</div><div className="text-[13px] font-mono font-bold text-foreground">{reads.bullish} / {reads.bearish}</div></div>
          <div className="border border-border bg-surface-elevated/30 p-2"><div className="text-[9px] font-mono text-muted-foreground">ADDING / CUTTING</div><div className="text-[13px] font-mono font-bold text-foreground">{reads.added} / {reads.reduced}</div></div>
          <div className="border border-border bg-surface-elevated/30 p-2"><div className="text-[9px] font-mono text-muted-foreground">NET SPEC %OI</div><div className={`text-[13px] font-mono font-bold ${reads.totalOi && rows.reduce((sum, row) => sum + row.managedMoney, 0) >= 0 ? 'text-positive' : 'text-negative'}`}>{formatPct(rows.reduce((sum, row) => sum + row.managedMoney, 0) / totalOi * 100)}</div></div>
          <div className="border border-border bg-surface-elevated/30 p-2"><div className="text-[9px] font-mono text-muted-foreground">AVG %ILE</div><div className="text-[13px] font-mono font-bold text-accent">{Math.round(rows.reduce((sum, row) => sum + row.pctRank, 0) / Math.max(1, rows.length))}%</div></div>
        </div>
      </Panel>
...
function DealerReport({ rows }: { rows: ReturnType<typeof groupRows> }) {
  return <SimpleReportTable title="Dealer / Swap Desk Pressure by Sector" rows={rows.map(row => ({ asset: row.asset, producerNet: row.commercials, swapProxy: Math.round(row.commercials * -0.28), moneyManager: row.managedMoney, oiShare: formatPct(row.openInterest / Math.max(1, rows.reduce((sum, r) => sum + r.openInterest, 0)) * 100), read: row.commercials < 0 ? 'Producer hedging pressure' : 'Commercial accumulation' }))} />;
}
...
function MarketsDetail({ rows, historyRows, selected, onSelect, selectedTicker, isPinned, toggleWatch }: { rows: COTLiveRow[]; historyRows: COTLiveRow[]; selected: COTLiveRow; onSelect: (ticker: string) => void; selectedTicker: string; isPinned: (row: COTLiveRow) => boolean; toggleWatch: (ticker: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <div className="border border-border bg-surface-deep p-3 xl:col-span-2">
          <div className="text-[10px] font-mono text-muted-foreground mb-1">{selected.ticker} Position History — Managed vs Commercial (Thousands)</div>
          <ExpandableResponsiveContainer width="100%" height={210}>
            <LineChart data={history}>
              ...
              <YAxis tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v}K`} />
              <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', fontFamily: 'monospace', fontSize: 10 }} formatter={(v: number) => [`${v}K`]} />
              <Line type="monotone" dataKey="managed"    stroke="hsl(var(--accent))"    strokeWidth={2} dot={false} name="Managed Money" />
              <Line type="monotone" dataKey="commercial" stroke="hsl(var(--negative))"  strokeWidth={2} dot={false} name="Commercials" />
            </LineChart>
          </ExpandableResponsiveContainer>
        </div>
        <div className="border border-border bg-surface-deep p-3">
          <div className="text-[10px] font-mono text-muted-foreground mb-1">Asset Peer Net Positions — {selected.asset}</div>
          <ExpandableResponsiveContainer width="100%" height={210}>
            <BarChart data={peerRows}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
              <XAxis dataKey="ticker" tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} />
              <YAxis tick={{ fontSize: 9, fontFamily: 'monospace', fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v / 1000}K`} />
              <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', fontFamily: 'monospace', fontSize: 10 }} formatter={(v: number) => [formatK(v)]} />
              <Bar dataKey="managedMoney">{peerRows.map((r, i) => <Cell key={i} fill={r.managedMoney >= 0 ? 'hsl(var(--positive)/0.7)' : 'hsl(var(--negative)/0.7)'} />)}</Bar>
            </BarChart>
          </ExpandableResponsiveContainer>
        </div>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Panel title="Granular Selected Contract">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-2">
            ...
            ] as const).map(([label, value]) => <div key={label} className="border border-border bg-surface-elevated/30 p-2"><div className="text-[9px] font-mono text-muted-foreground">{label}</div><div className="text-[11px] font-mono font-bold text-foreground">{value}</div></div>)}
          </div>
        </Panel>
        <Panel title="Dealer / Producer Pressure">
          <table className="w-full text-[10px] font-mono">
            <thead><tr className="border-b border-border"><th className="text-left px-2 py-1 text-muted-foreground">MKT</th><th className="text-right px-2 py-1 text-muted-foreground">PROD</th><th className="text-right px-2 py-1 text-muted-foreground">SWAP</th><th className="text-right px-2 py-1 text-muted-foreground">MM</th><th className="text-left px-2 py-1 text-muted-foreground">READ</th></tr></thead>
            <tbody>{dealerPressure.map((row, i) => <tr key={row.market} className={`border-b border-border/40 ${i % 2 ? 'bg-surface-elevated/20' : ''}`}><td className="px-2 py-1 text-accent font-bold">{row.market}</td><td className={`px-2 py-1 text-right font-bold ${row.producer >= 0 ? 'text-positive' : 'text-negative'}`}>{row.producer}</td><td className="px-2 py-1 text-right text-muted-foreground">{row.swapDealer}</td><td className={`px-2 py-1 text-right font-bold ${row.moneyManager >= 0 ? 'text-positive' : 'text-negative'}`}>{row.moneyManager}</td><td className="px-2 py-1 text-muted-foreground">{row.read}</td></tr>)}</tbody>
          </table>
        </Panel>
        <Panel title="Tenor Concentration">
          <table className="w-full text-[10px] font-mono">
            <thead><tr className="border-b border-border"><th className="text-left px-2 py-1 text-muted-foreground">BUCKET</th><th className="text-right px-2 py-1 text-muted-foreground">ENERGY</th><th className="text-right px-2 py-1 text-muted-foreground">METALS</th><th className="text-right px-2 py-1 text-muted-foreground">GRAINS</th><th className="text-right px-2 py-1 text-muted-foreground">RATES</th></tr></thead>
            <tbody>{termStructure.map((row, i) => <tr key={row.bucket} className={`border-b border-border/40 ${i % 2 ? 'bg-surface-elevated/20' : ''}`}><td className="px-2 py-1 text-accent font-bold">{row.bucket}</td><td className="px-2 py-1 text-right text-foreground">{row.energy}</td><td className="px-2 py-1 text-right text-foreground">{row.metals}</td><td className="px-2 py-1 text-right text-foreground">{row.grains}</td><td className="px-2 py-1 text-right text-foreground">{row.rates}</td></tr>)}</tbody>
          </table>
        </Panel>
      </div>
...
function MarketsTable({ rows, title = 'Futures Positioning', compact = false, onSelect, selectedTicker, allRows, historyRows, isPinned, toggleWatch }: { rows: COTLiveRow[]; title?: string; compact?: boolean; onSelect: (ticker: string) => void; selectedTicker: string; allRows: COTLiveRow[]; historyRows: COTLiveRow[]; isPinned: (row: COTLiveRow) => boolean; toggleWatch: (ticker: string) => void }) {
  ...
  return (
    <Panel title={title} action={<button onClick={() => downloadCSV(`${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.csv`, rows.map(r => ({ ticker: r.ticker, market: r.market, asset: r.asset, openInterest: r.openInterest, managedMoney: r.managedMoney, commercials: r.commercials, week: r.week, fourWeek: r.fourWeek, pctRank: r.pctRank, bias: r.bias })))} className="text-[9px] font-mono text-muted-foreground hover:text-accent flex items-center gap-1"><Download className="h-3 w-3" /> CSV</button>}>
      ...
      {compact && <div className="px-2 py-1 text-[9px] font-mono text-muted-foreground border-t border-border">Showing contracts above 80th or below 20th percentile.</div>}
    </Panel>
  );
}

function WatchlistView({ rows, watchlist, onSelect, toggleWatch, historyRows }: { rows: COTLiveRow[]; watchlist: string[]; onSelect: (ticker: string) => void; selectedTicker: string; toggleWatch: (ticker: string) => void; historyRows: COTLiveRow[] }) {
    ...
    <div className="space-y-3">
      <Panel title={`Watchlist · ${watchedRows.length} contracts`}>
        {watchedRows.length === 0 ? (
          <div className="p-4 text-center text-[10px] font-mono text-muted-foreground">No starred contracts. Click ★ on any row, or press <span className="text-accent">F</span> with a ticker selected.</div>
        ) : (
          <SortableFilterTable
            rows={watchedRows}
            columns={[
            ...
            rowKey={r => r.ticker} onRowClick={r => onSelect(r.ticker)} minWidth={1020}
          />
        )}
      </Panel>
      <Panel title="Add to Watchlist · Unstarred Markets">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-1 p-2">
          {unwatched.map(r => (
            <button key={r.ticker} onClick={() => toggleWatch(r.ticker)} className="border border-border bg-surface-elevated/30 p-1.5 text-left hover:border-accent/60 hover:bg-surface-elevated">
              <div className="flex items-center justify-between"><span className="text-accent font-mono font-bold text-[10px]">{r.ticker}</span><Star className="h-3 w-3 text-muted-foreground" /></div>
...
function CorrelationMatrix({ history, latestRows, onSelect }: { history: COTLiveRow[]; latestRows: COTLiveRow[]; onSelect: (ticker: string) => void }) {
  ...
  const universe = (assetFilter === 'All' ? latestRows : latestRows.filter(r => r.asset === assetFilter)).sort((a, b) => b.openInterest - a.openInterest).slice(0, 14);
  ...
  const cellColor = (r: number) => { const abs = Math.min(1, Math.abs(r)); const hue = r >= 0 ? 'var(--positive)' : 'var(--negative)'; return `hsl(${hue} / ${0.15 + abs * 0.65})`; };
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] font-mono text-muted-foreground">FILTER:</span>
        {assets.map(a => <button key={a} onClick={() => setAssetFilter(a)} className={`px-2 py-1 border text-[9px] font-mono ${assetFilter === a ? 'border-accent text-accent bg-accent/10' : 'border-border text-muted-foreground hover:border-accent/50'}`}>{a}</button>)}
        <span className="ml-auto text-[9px] font-mono text-muted-foreground">{hasData ? 'Rolling ~26w · MM net %OI' : 'Need stored COT history — feed will populate after first fetch.'}</span>
      </div>
      <Panel title="Positioning Correlation Matrix">
        <div className="overflow-x-auto">
          <table className="font-mono text-[9px]" style={{ minWidth: 32 * (tickers.length + 1) }}>
            <thead><tr><th className="px-1 py-0.5"></th>{tickers.map(t => <th key={t} className="px-1 py-0.5 text-accent font-bold cursor-pointer" onClick={() => onSelect(t)}>{t}</th>)}</tr></thead>
            <tbody>
              {tickers.map(rowT => (
                <tr key={rowT}>
                  <td className="px-1 py-0.5 text-accent font-bold cursor-pointer" onClick={() => onSelect(rowT)}>{rowT}</td>
                  {tickers.map(colT => { const cell = matrix.find(c => c.a === rowT && c.b === colT); const r = cell?.r ?? 0; return <td key={colT} title={`${rowT} ↔ ${colT}: ${r.toFixed(2)}`} className="px-1 py-0.5 text-center" style={{ backgroundColor: hasData ? cellColor(r) : 'transparent' }}>{hasData ? r.toFixed(1).replace('0.', '.').replace('-.', '-.') : '·'}</td>; })}
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Panel title="Most Correlated Pairs (26w)">
          <table className="w-full text-[10px] font-mono"><thead><tr className="border-b border-border"><th className="text-left px-2 py-1 text-muted-foreground">PAIR</th><th className="text-right px-2 py-1 text-muted-foreground">ρ</th></tr></thead><tbody>{topPos.map(p => <tr key={`${p.a}-${p.b}`} className="border-b border-border/40 hover:bg-surface-elevated cursor-pointer" onClick={() => onSelect(p.a)}><td className="px-2 py-1 text-accent font-bold">{p.a} ↔ {p.b}</td><td className="px-2 py-1 text-right text-positive font-bold">{p.r.toFixed(2)}</td></tr>)}{!topPos.length && <tr><td colSpan={2} className="px-2 py-3 text-center text-muted-foreground">Awaiting history</td></tr>}</tbody></table>
        </Panel>
        <Panel title="Most Decorrelated Pairs (26w)">
          <table className="w-full text-[10px] font-mono"><thead><tr className="border-b border-border"><th className="text-left px-2 py-1 text-muted-foreground">PAIR</th><th className="text-right px-2 py-1 text-muted-foreground">ρ</th></tr></thead><tbody>{topNeg.map(p => <tr key={`${p.a}-${p.b}`} className="border-b border-border/40 hover:bg-surface-elevated cursor-pointer" onClick={() => onSelect(p.a)}><td className="px-2 py-1 text-accent font-bold">{p.a} ↔ {p.b}</td><td className="px-2 py-1 text-right text-negative font-bold">{p.r.toFixed(2)}</td></tr>)}{!topNeg.length && <tr><td colSpan={2} className="px-2 py-3 text-center text-muted-foreground">Awaiting history</td></tr>}</tbody></table>
        </Panel>
      </div>
    </div>
  );
...
function KpiBox({ label, value, sub, tone }: { label: string; value: string; sub: string; tone: string }) {
  return (
