...
import TickerQuoteInline, { extractTicker } from '@/components/news/TickerQuoteInline';
...
export default function NewsDetailPanel(p: Props) {
  const [tab, setTab] = useState<Tab>(p.initialTab ?? 'detail');
  useEffect(() => { if (p.initialTab) setTab(p.initialTab); }, [p.initialTab]);

  const a = p.active;
  const detectedTicker = a ? extractTicker(a.title) : null;
  const macroCtx = useMacroContext(a?.topic ?? '');

  return (
    <div className={`flex flex-col bg-background border-l border-border font-mono ${p.className ?? ''}`}>
      <div className="flex items-center px-2 py-0.5 border-b border-border bg-surface-deep flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {tab === 'detail' && (a ? (
          <div className="p-2 space-y-3">
            ...
            {detectedTicker && (
              <TickerQuoteInline ticker={detectedTicker} />
            )}

            {macroCtx.length > 0 && (
              <div className="flex gap-3 text-[9px] font-mono px-1 py-0.5 border border-border/40 bg-surface-deep">
                {macroCtx.map(item => (
                  <span key={item.key} className="flex items-center gap-1">
                    <span className="text-muted-foreground uppercase">{item.label}</span>
                    <span className={`font-bold ${(item.change ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
