// Federal Reserve speeches + press releases RSS aggregator.
// Output matches gdelt-news article shape so the frontend can merge.
...
const FEEDS = [
  { url: "https://www.federalreserve.gov/feeds/speeches.xml", domain: "federalreserve.gov", kind: "speech" },
  { url: "https://www.federalreserve.gov/feeds/press_all.xml", domain: "federalreserve.gov", kind: "press" },
];

const SPEAKERS = ["Powell", "Williams", "Waller", "Jefferson", "Cook", "Kugler", "Bowman", "Barr", "Brainard", "Harker", "Bostic", "Daly", "Mester", "Kashkari", "Goolsbee", "Logan", "Schmid"];

function detectSpeaker(title: string): string | undefined {
  for (const s of SPEAKERS) if (title.includes(s)) return s;
  return undefined;
}
...
async function fetchFeed(f: typeof FEEDS[number]): Promise<Article[]> {
  try {
    ...
    return parseRss(xml).slice(0, 25).map((it): Article => ({
      ...
      tier: 1,
      topic: "central-bank",
      speaker: detectSpeaker(it.title),
    }));
  } catch {
    return [];
  }
